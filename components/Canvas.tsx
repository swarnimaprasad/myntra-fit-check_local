...
const Canvas: React.FC<CanvasProps> = ({ 
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      ...
      <button 
          onClick={onStartOver}
          className="absolute top-4 left-4 z-30 flex items-center justify-center text-center bg-white border border-gray-300 text-gray-800 font-bold py-2 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-gray-100 hover:border-gray-400 active:scale-95 text-xs uppercase tracking-wider backdrop-blur-sm"
      >
        ...
        {isComparing && comparisonImageUrl && displayImageUrl ? (
          ...
          >
            <button
              ...
              className="absolute top-4 right-4 z-30 flex items-center justify-center text-center bg-white border border-gray-300 text-gray-800 font-bold py-2 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-gray-100 hover:border-gray-400 active:scale-95 text-xs uppercase tracking-wider"
            >
            ...
            <div className="w-full h-full max-w-5xl aspect-[16/9] relative">
              <Compare
                ...
                className="w-full h-full rounded-2xl bg-gray-200 shadow-lg"
              />
            </div>
          </motion.div>
        ) : (
          ...
          >
            ...
            {displayImageUrl && !isLoading && (
              ...
              >
                ...
                <AnimatePresence>
                    {isPoseMenuOpen && (
                        <motion.div
                            ...
                            className="absolute bottom-full mb-3 w-64 bg-white/90 backdrop-blur-lg rounded-xl p-2 border border-gray-200/80 shadow-lg"
                        >
                            <div className="grid grid-cols-2 gap-2">
                                {poseInstructions.map((pose, index) => (
                                    <button
                                        ...
                                        className="w-full text-left text-sm font-medium text-gray-800 p-2 rounded-md hover:bg-primary-50 hover:text-primary-700 disabled:opacity-50 disabled:bg-primary-50 disabled:text-primary-700 disabled:font-bold disabled:cursor-not-allowed"
                                    >
                                        {pose}
                                    </button>
                                ))}
                ...
                <div className="flex items-center justify-center gap-2 bg-white/80 backdrop-blur-md rounded-full p-1 border border-gray-200/80 shadow-md">
                  <button 
                    onClick={handlePreviousPose}
                    aria-label="Previous pose"
                    className="p-2 rounded-full hover:bg-primary-50 active:scale-90 transition-all disabled:opacity-50"
                    disabled={isLoading}
                  >
